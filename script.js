const Modal = {
  ...
  const DOM = {
      ...
      innerHTMLTransaction(transactions, index){
          ...
          <td>
            <img onclick="Transactions.remove(${index})" src="./assets/assets/minus.svg" alt="Remover Transação">
  ...
  const Form = {
      ...
      submit(event){
          ...
          try {
            ...
            //fechar o modal
            Modal.close()
            //is not necessary to reload the app because the app is already reloaded when a new transaction is added
          } catch (error) {
            alert(error.message)  
          }
